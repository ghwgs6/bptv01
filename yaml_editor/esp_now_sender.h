...
class ESPNowSenderComponent : public esphome::Component {
 public:
  void setup() override {
    ...
    ESP_LOGI("esp_now", "ESP-NOW Sender Initialized! Peers: Board 2 & 7\" P4 Display added.");
  }

  void send_state(const char* state_str) {
    // Broadcast / Send to both peers
    ...
    ESP_LOGI("esp_now", "Sent ESP-NOW packet to Board 2 & 7\" Display: %s", state_str);
  }
};
